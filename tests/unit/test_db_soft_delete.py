"""
Unit tests for soft-delete (deleted_at column) + audit_log migration, and the
conversion of delete_category/delete_video_session from hard DELETE to
soft-delete. Scope: only these two entities (and video_sessions' cascaded
segments) actually get soft-deleted in this change — the other tables gain
the deleted_at column now (so a later change doesn't need another
migration) but no read filtering was added for them since nothing sets it.

RED phase: deleted_at/audit_log do not exist yet; delete_category and
delete_video_session still hard-DELETE.
"""
